...
const CreateExpenseDialog = ({ updateState }: CreateExpenseProps) => {
	const { register, handleSubmit } = useForm<ExpenseDTO>();
	...
	const handleExpenseCreation = async (data: ExpenseDTO) => {
		setDisabled(true);
		mutate({
		...
		<Dialog.Portal>
			...
			<Dialog.Content className="bg-darkbg fixed top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/4 px-8 py-7 lg:w-96 max-md:w-80 rounded-lg">
				...
				<form action="" onSubmit={handleSubmit(handleExpenseCreation)}>
					...
					<Dialog.Close
						className={
							"w-full bg-red-600 text-white mt-4 px-3 py-4 rounded transition-all  uppercase font-bold disabled:opacity-60 disabled:cursor-not-allowed"
						}
					>
						Cancelar
					</Dialog.Close>
