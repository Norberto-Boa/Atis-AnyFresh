  ...
  async function handleEditSale({
    ...
    try {
      await api
        ...
        .then(() => {
          alert(
            `Venda da ${sale.client_name} editada com sucesso! ${sale.discount}`
          );
          router.push(`/sales`);
        });
    } catch (error) {
    ...
    <div className={`ml-80 pt-16 text-white`}>
      ...
      <div className="p-16">
        <h1 className={`text-2xl font-semibold`}>
          Editar a venda feita para{" "}
          {`${sale.client_name} - no dia ${sale.date}`};
        </h1>

        <h2 className={`mt-4 font-semibold text-xl`}></h2>
        ...
        <div className="flex gap-32">
          <form className="w-1/3" onSubmit={handleSubmit(handleEditSale)}>
            <div className="mb-2">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-wide">Dados Actuais</h2>
